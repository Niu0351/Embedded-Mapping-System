...
void PortK0_7_Init(void){                                // output for motor 4-7 
...
while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R9) == 0){};        //allow time for clock to stabilize
	GPIO_PORTK_DIR_R = 0b11110000;          							 
  GPIO_PORTK_DEN_R = 0b11110000;
return;
}

void PortM_Init(void){                                // output for motor 4-7 
SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R11;                 //activate the clock for Port k
...
void PortF0_Init (void){                                // angle LED internal LED
SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R5;                 //activate the clock for Port k
...
SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R10;                 //activate the clock for Port k
...
int main(void) {
  ...
  uint8_t RangeStatus;
  uint8_t dataReady;
	int step = 64;                                       // step = 512/360*angle
	...
	while(1){
		...
		if (((GPIO_PORTM_DATA_R&0b00000001)==0)){
			...
			while ( count < 512 ){
				...
				GPIO_PORTK_DATA_R |= 0b00110000; // b' a' b a        
				SysTick_Wait10ms(1);
				GPIO_PORTK_DATA_R &= 0b00000000;
				GPIO_PORTK_DATA_R |= 0b01100000;
				SysTick_Wait10ms(1);
				GPIO_PORTK_DATA_R &= 0b00000000;
				GPIO_PORTK_DATA_R |= 0b11000000;
				SysTick_Wait10ms(1);
				GPIO_PORTK_DATA_R &= 0b00000000;
				GPIO_PORTK_DATA_R |= 0b10010000;
				SysTick_Wait10ms(1);
				count++;
				...
				if (count%(step)==0){
					...
					debugArray[j] = Distance; 
					//UART_OutChar(debugArray[j]);
					
			//	  status = VL53L1X_GetSignalRate(dev, &SignalRate);
			//	  status = VL53L1X_GetAmbientRate(dev, &AmbientRate);
			//	  status = VL53L1X_GetSpadNb(dev, &SpadNum);
					status = VL53L1X_ClearInterrupt(dev); /* clear interrupt has to be called to enable next interrupt*/
					//sprintf(printf_buffer,"%u, %u, %u, %u, %u\r\n", RangeStatus, Distance, debugArray[j]/*SignalRate*/, AmbientRate,SpadNum);
					//UART_printf(printf_buffer);
...
//}
  //int input=0;
	//sprintf(printf_buffer,"Start Transmission\n");
	//UART_printf(printf_buffer);
